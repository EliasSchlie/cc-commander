...
beforeEach(() => {
  db = new HubDb(":memory:");
  auth = new AuthService(db, "test-secret-key");
});
afterEach(() => {
  db.close();
});
...
describe("login", () => {
  ...
  // Prevents: login for non-existent account leaking info
  it("rejects unknown email", async () => {
    await assert.rejects(
      () => auth.login("nobody@test.com", "pass"),
      /Invalid credentials/,
    );
  });
});
...
describe("verifyToken", () => {
  ...
  // Prevents: tampered tokens being accepted
